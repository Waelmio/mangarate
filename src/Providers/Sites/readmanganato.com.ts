...
export class ReadmanganatoProviderInfo extends MangaInfoProvider {
    ...
    async getAllChapters(): Promise<BaseChapter[]> {
        try {
            const cheerioDoc = cheerio.load(this.contentPageHtml);
            const chapterListPath = "body > div.body-site > div.container.container-main > div.container-main-left > div.panel-story-chapter-list";
            ...
            for (let i = chapterAmount; i >= 1; --i) {
                ...
                const chapterUrl = cheerioChaps(jsChapterUrlPath).attr('href');
