...
export class Watcher {
    ...
    public start(): void {
        ...
        this._interval = config.get("watcher.interval");
        ...
        this._timer = setInterval(this.watchRandomizer.bind(this),
            Math.floor(
                this._interval * 1000 * 60 * 0.95
            )
        );
    }
    ...
    private async watchRandomizer(): Promise<void> {
        try {
            const randomTime = Math.floor(
                this._interval * 1000 * 60 * (Math.random() * 0.1)
            );
